...
const manifest = JSON.parse(fs.readFileSync(path.join(root, "config", "v2-02-source-set.json"), "utf8"));

test("V2-02 workflow cannot silently keep accepting the old V2-01 source set", () => {
  const workflow = fs.readFileSync(path.join(root, ".github", "workflows", "e2e.yml"), "utf8");
  assert.equal(fs.existsSync(path.join(root, "config", "v2-02-source-set.json")), true, "V2-02 needs its own immutable source-set manifest");
  ...
  assert.match(workflow, /config\/v2-02-source-set\.json/);
});

test("V2-02 verifier rejects the immutable V2-01 manifest even when explicitly selected", () => {
  const result = spawnSync(process.execPath, [path.join(root, "scripts", "verify-source-set.mjs"), "--allow-dirty"], {
    encoding: "utf8",
    env: { ...process.env, SOURCE_SET_MANIFEST: path.join(root, "config", "v2-01-source-set.json") },
  });
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Unsupported source-set release: expected V2-02/);
});

test("V2-02 source set declares exact immutable O/P/W/K/L revisions", () => {
  assert.equal(manifest.schemaVersion, "dsdst.test-source-set.v1");
  assert.equal(manifest.release, "V2-02");
  ...
  assert.deepEqual(Object.fromEntries(["O", "P", "W", "K", "L"].map((id) => [id, byId.get(id).revision])), {
    O: "6c944b9bcbc5d9cad087b66683b5aa9a736ba0b3",
    P: "fa58bf8e703b33a758bd5e81a274f06275490f09",
    W: "6a3819629b66b83b0ee6aecacdd3cae606951a16",
    K: "c76904d38921a39d8d791b5ef1ec0c232df9bd45",
    L: "12219ae925690238c7f6f1bb073d7c9ccd97a88f",
  });
});
...
test("source-set verifier rejects self-resolved Operations revisions", () => {
  ...
  try {
    writeFileSync(manifestPath, JSON.stringify({ schemaVersion: "dsdst.test-source-set.v1", release: "V2-02", repositories: entries }));
    const result = spawnSync(process.execPath, [path.join(root, "scripts", "verify-source-set.mjs"), "--allow-dirty"], {
      encoding: "utf8",
      env: {
        ...process.env,
